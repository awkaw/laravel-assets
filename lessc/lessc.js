...
var continueProcessing = true; // Calling process.exit does not flush stdout always. Instead of exiting the process, we set the process' exitCode,
...
function render() {
  ...
  var ensureDirectory = function ensureDirectory(filepath) {
    ...
    var cmd;
    var existsSync = fs$1.existsSync || path.existsSync;
    ...
    if (!existsSync(dir)) {
      ...
      cmd = mkdirp && mkdirp.sync || fs$1.mkdirSync;
      cmd(dir);
    }
  };
  ...
  if (!sourceMapFileInline) {
    var writeSourceMap = function writeSourceMap() {
      ...
      fs$1.writeFile(filename, output, 'utf8', function (err) {
        if (err) {
  ...
  var writeOutput = function writeOutput(output, result, onSuccess) {
    if (options.depends) {
      ...
      fs$1.writeFile(output, result.css, {
        encoding: 'utf8'
      }, function (err) {
        if (err) {
          var description = 'Error: ';
  ...
  if (input != '-') {
    fs$1.readFile(input, 'utf8', parseLessFile);
  } else {
  ...
  args = args.filter(function (arg) {
    ...
    switch (arg) {
      ...
      case 'relative-urls':
        ...
        options.rewriteUrls = RewriteUrls.ALL;
        break;
      ...
      case 'rewrite-urls':
        ...
        if (m) {
          if (m === 'local') {
            options.rewriteUrls = RewriteUrls.LOCAL;
          } else if (m === 'off') {
            options.rewriteUrls = RewriteUrls.OFF;
          } else if (m === 'all') {
            options.rewriteUrls = RewriteUrls.ALL;
          } else {
      ...
      case 'strict-math':
        ...
        if (checkArgFunc(arg, match[2])) {
          if (checkBooleanArg(match[2])) {
            options.math = Math$1.STRICT_LEGACY;
          }
        }
